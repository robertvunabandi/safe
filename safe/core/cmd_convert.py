...
def run(
    converter: Converter, filepath: Path, decrypt: bool, overwrite: bool = False
) -> int:
    if not filepath.exists():
        # todo - somehow print error messages to tell that the file doesn't exist
        return exit_codes.FILE_DOES_NOT_EXIST
    if decrypt:
        return _run_decryption(converter, filepath, overwrite)
    return _run_encryption(converter, filepath, overwrite)


def _run_encryption(converter: Converter, filepath: Path, overwrite: bool) -> int:
...
def _run_decryption(converter: Converter, filepath: Path, overwrite: bool) -> int:
